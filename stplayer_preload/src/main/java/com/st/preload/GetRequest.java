...
public class GetRequest {
    ...
    private static final Pattern PREFETCH_SIZE_PATTERN = Pattern.compile("ST_PREFETCH_MAX_SIZE:[ ]?(\\d*)");
    ...
    public GetRequest(String request) {
        ...
        this.prefetchSize = findPrefetchSize(request);
    }
    ...
    private long findPrefetchSize(String request) {
        Matcher matcher = PREFETCH_SIZE_PATTERN.matcher(request);
        if (matcher.find()) {
            String rangeValue = matcher.group(1).trim();
            try {
                return Long.parseLong(rangeValue);
